...
def parse(String description) {
	...
	if (result) { 
		//log.debug "dbg: parse(): received ${description} => return ${result?.descriptionText}"
		return result
	}
}
...
def zwaveEvent(physicalgraph.zwave.commands.meterv1.MeterReport cmd) {
	...
	if (cmd.meterType == 33) {
		if (cmd.scale == 0) {
			...
			if (DEBUG) { log.debug "dbg: zwaveEvent(cmd): meterType=${cmd.meterType} scale=${cmd.scale} newValue=${newValue} kWhCost=${kWhCost}" }
